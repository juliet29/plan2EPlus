...
def initial_room(room_def: RoomDefinition): 
    return RoomFromJSON(0, room_def.label, 0, 0, str(room_def.width), str(room_def.height))

def adjacent_right(adj_room: RoomFromJSON, room_def: RoomDefinition):
    top = adj_room.top
    left = adj_room.create_domain().width.max
    id = adj_room.id
    return RoomFromJSON(id, room_def.label, left, top, str(room_def.width), str(room_def.height))
