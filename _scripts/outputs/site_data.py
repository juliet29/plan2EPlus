...
from outputs.classes import SiteData
...
class SiteDataPlots(SQLReader):
    ...
    def plot_all_site_data(self, ap_index):
        ...
        for ix, output in enumerate(self.site_data.values()):
            ...
            dataset = data.dataset
            fig.add_trace(
                go.Scatter(x=dataset.datetimes, y=dataset.values), row=ix + 1, col=1
            )
            fig.update_yaxes(
                title_text=f"{data.type} [{data.unit}]",
                row=ix + 1,
                col=1,
        ...
        fig.update_layout(
            title_text=f"{analysis_period} {self.site_name}", showlegend=False
        )
