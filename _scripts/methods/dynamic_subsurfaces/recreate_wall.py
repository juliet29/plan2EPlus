...
class WallRecreation:
    ...
    def create_polygon(self):
        c1 = get_point_as_xy(self.coords[0]) + (0,)
        c2 = get_point_as_xy(self.coords[1]) + (0,)

        c3 = get_point_as_xy(self.coords[0]) + (self.room_height,)
        c4 = get_point_as_xy(self.coords[1]) + (self.room_height,)
    ...
    def is_x_dir(self):
        if (
            self.wall.direction == WallNormal.NORTH.name
            or self.wall.direction == WallNormal.SOUTH.name
        ):
    ...
    def is_y_dir(self):
        if (
            self.wall.direction == WallNormal.EAST.name
            or self.wall.direction == WallNormal.WEST.name
        ):
